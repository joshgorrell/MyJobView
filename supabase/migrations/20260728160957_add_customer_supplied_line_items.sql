/*
# Add "Customer Supplied" flag to proposal line items

## Purpose
Allows marking a line item as "Customer Supplied" — the customer is
providing the item themselves, so it should NOT be priced or included
in any totals, tax, or deposit calculations. The item still appears in
the scope of work (description + quantity) but shows $0 / "—" pricing.

## Changes

### 1. New column on proposal_line_items
- `is_customer_supplied` (boolean, NOT NULL, DEFAULT false)

### 2. Updated trigger: auto_compute_line_item_totals
- When `is_customer_supplied = true`, force `unit_price = 0`,
  `line_total = 0`, `labor_total = 0`, and `labor_hours = 0` /
  `labor_rate = 0` so the item never contributes to any financial
  total.

### 3. Updated trigger: calculate_proposal_line_item_labor
- Same zeroing behavior when `is_customer_supplied = true`.

### 4. Updated function: calculate_proposal_totals
- Parts total query: exclude items where `is_customer_supplied = true`
- Labor total query: exclude items where `is_customer_supplied = true`
- This ensures customer-supplied items never affect subtotal, tax,
  modifiers, or deposit.

### 5. Backfill
- Recalculate all existing proposal totals so any future
  customer-supplied items are correctly excluded.
*/
...
-- Step 2: Update auto_compute_line_item_totals to zero out customer-supplied items
...
AS $$
BEGIN
  -- If customer-supplied, force all pricing to zero
...
-- Step 3: Update calculate_proposal_line_item_labor to zero out customer-supplied items
...
-- Step 4: Update calculate_proposal_totals to exclude customer-supplied items
...
BEGIN
  ...
  -- Parts total: exclude customer-supplied items
  ...
  -- Labor total: exclude customer-supplied items
  SELECT
    COALESCE(SUM(
      CASE
...
-- Step 5: Recalculate all existing proposals
DO $$
DECLARE
  proposal_rec RECORD;
BEGIN
