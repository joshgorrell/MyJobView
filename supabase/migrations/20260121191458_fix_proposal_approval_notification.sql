/*
  # Fix Proposal Approval Notification

  1. Issue
    - Trigger tries to insert into non-existent 'activity_feed' table
    - Should use 'notifications' table instead

  2. Changes
    - Update trigger to insert into 'notifications' table with correct columns
    - Use related_id to link to proposal
*/

-- Fix the approval trigger to use notifications table instead of activity_feed
...
COMMENT ON FUNCTION create_sales_order_from_proposal() IS 'Creates sales order when proposal is approved and sends notification to creator';