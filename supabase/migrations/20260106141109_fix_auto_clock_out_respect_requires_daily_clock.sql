/*
  # Fix Auto Clock Out - Respect requires_daily_clock
  
  1. Changes
    - Update auto_clock_out_forgotten_entries() to only process users who require daily clock tracking
    - Skip users with requires_daily_clock = false (admin, office staff, salary employees, etc.)
*/
...
BEGIN
  ...
  -- ONLY for users who require daily clock tracking
  FOR v_entry IN
    SELECT 
      dce.id,
      dce.technician_id,
    ...
    AND p.requires_daily_clock = true  -- Only process users who need daily clock tracking
  LOOP
