/*
  # Fix approval trigger guard condition to allow retry when SO is missing

  ## Problem
  The `handle_unified_proposal_approval` trigger has this guard condition:
  
    AND NEW.billing_action_taken = false

  If a proposal gets into a broken state where `billing_action_taken=true` but
  `sales_order_id IS NULL` (e.g., from a previous failed attempt or the ON DELETE SET NULL
  cascade from a deleted sales order), the trigger will never re-run. The proposal becomes
  permanently stuck — it cannot be approved again.
  ...
  This allows the trigger to re-run if there's no sales order, regardless of the
  billing_action_taken flag, ensuring stuck proposals can always be recovered.
*/
