/*
  # Fix submit_security_onboarding: cannot update generated column full_name

  ## Problem
  The `full_name` column on `contacts` is a GENERATED ALWAYS column computed from
  first_name / last_name. The previous RPC tried to SET full_name directly, which
  Postgres rejects with "column can only be updated to DEFAULT".

  ## Fix
  Split the incoming p_full_name into first_name / last_name (on the first space)
  and update those columns instead. Also keep contact_name in sync as a fallback.
*/
