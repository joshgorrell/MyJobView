/*
  # Fix Proposal Approval Trigger - Settings Lookup

  1. Issue
    - Trigger tries to reference NEW.proposal_settings_id which doesn't exist
    - proposals table doesn't have proposal_settings_id foreign key
    - proposal_settings is company-wide configuration

  2. Fix
    - Lookup proposal_settings by company_id instead
    - Get first/default proposal_settings record for the company
    - Use sensible defaults if no settings exist
*/

-- Fix the approval trigger to lookup settings by company_id
...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status != 'approved' AND NEW.sales_order_id IS NULL THEN
    ...
    -- Get proposal-level acceptance requirements from company's proposal settings
    SELECT 
    ...
    -- If no proposal_settings found, use defaults
...
COMMENT ON FUNCTION create_sales_order_from_proposal() IS 'Creates sales order when proposal is approved, looking up settings by company_id';
