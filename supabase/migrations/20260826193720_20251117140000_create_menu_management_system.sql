/*
  # Menu Management System (idempotent re-creation)
*/
...
DROP POLICY IF EXISTS "Anyone authenticated can view enabled menu items" ON menu_items;
CREATE POLICY "Anyone authenticated can view enabled menu items" ON menu_items FOR SELECT TO authenticated USING (is_enabled = true);
DROP POLICY IF EXISTS "Admins can insert menu items" ON menu_items;
CREATE POLICY "Admins can insert menu items" ON menu_items FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));
DROP POLICY IF EXISTS "Admins can update non-system menu items" ON menu_items;
CREATE POLICY "Admins can update non-system menu items" ON menu_items FOR UPDATE TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));
DROP POLICY IF EXISTS "Admins can delete non-system menu items" ON menu_items;
CREATE POLICY "Admins can delete non-system menu items" ON menu_items FOR DELETE TO authenticated USING (is_system = false AND EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));
DROP POLICY IF EXISTS "Anyone authenticated can view menu role permissions" ON menu_item_roles;
CREATE POLICY "Anyone authenticated can view menu role permissions" ON menu_item_roles FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "Admins can manage menu role permissions" ON menu_item_roles;
CREATE POLICY "Admins can manage menu role permissions" ON menu_item_roles FOR ALL TO authenticated USING (EXISTS (SELECT 1 FROM profiles WHERE profiles.id = auth.uid() AND profiles.role = 'admin'));
...
DO $$
DECLARE myjobview_id uuid; recur_id uuid; settings_id uuid;
BEGIN
...
INSERT INTO menu_item_roles (menu_item_id, role, can_access) SELECT id, 'admin', true FROM menu_items WHERE parent_id IS NULL ON CONFLICT (menu_item_id, role) DO NOTHING;
INSERT INTO menu_item_roles (menu_item_id, role, can_access) SELECT id, 'sales', true FROM menu_items WHERE parent_id IS NULL AND key != 'settings' ON CONFLICT (menu_item_id, role) DO NOTHING;
INSERT INTO menu_item_roles (menu_item_id, role, can_access) SELECT id, 'office_manager', true FROM menu_items WHERE parent_id IS NULL ON CONFLICT (menu_item_id, role) DO NOTHING;
INSERT INTO menu_item_roles (menu_item_id, role, can_access) SELECT id, 'field_tech', true FROM menu_items WHERE parent_id IS NULL AND key IN ('feed', 'tasks', 'mycard', 'preferences') ON CONFLICT (menu_item_id, role) DO NOTHING;
INSERT INTO menu_item_roles (menu_item_id, role, can_access) SELECT m.id, 'admin', true FROM menu_items m INNER JOIN menu_items p ON m.parent_id = p.id WHERE p.key = 'settings' ON CONFLICT (menu_item_id, role) DO NOTHING;

CREATE OR REPLACE FUNCTION update_menu_items_updated_at() RETURNS TRIGGER AS $$ BEGIN NEW.updated_at = now(); RETURN NEW; END; $$ LANGUAGE plpgsql;
DROP TRIGGER IF EXISTS menu_items_updated_at ON menu_items;
CREATE TRIGGER menu_items_updated_at BEFORE UPDATE ON menu_items FOR EACH ROW EXECUTE FUNCTION update_menu_items_updated_at();