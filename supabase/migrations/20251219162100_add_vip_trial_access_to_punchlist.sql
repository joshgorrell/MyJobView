/*
  # Add VIP Trial Access to Punchlist System

  1. Summary
    - Updates punchlist access checking to include VIP memberships
    - Customers with active VIP subscriptions (including trial periods) get automatic punchlist access
    - Trial period "Test & Tune" access is free during the trial
    - Admin can extend trials to give customers more time

  2. Changes
    - Updates get_punchlist_access_info() to check both:
      * Manual punchlist_access_grants (existing system)
      * Active VIP subscriptions including trial periods (new)
    - VIP trial access takes priority over manual grants
    - Trial days remaining are calculated from subscription trial_end_date

  3. Access Priority (highest to lowest)
    1. VIP subscription (active or trial)
    2. Manual punchlist grants (test_and_tune)

  4. Security
    - Function remains SECURITY DEFINER with proper access controls
    - No changes to RLS policies
*/

-- Update the punchlist access info function to include VIP subscriptions
