/*
  # Fix Proposal Totals to Match Tax Report Calculation

  1. Problem
    - Database function treats labor-only items (item_type = 'labor') line_total as parts
    - Tax report treats labor-only items as labor only
    - This causes mismatch between footer totals and tax report

  2. Solution
    - Update calculate_proposal_totals to check item_type when calculating parts vs labor
    - For item_type = 'labor': line_total should go to labor_total
    - For item_type != 'labor' (material): line_total goes to parts_total, labor_total stays as labor

  3. Logic
    - Material items: parts = line_total, labor = labor_total
    - Labor-only items: parts = 0, labor = line_total (or labor_total if set)
*/
