/*
  # Fix Proposals View Relationship Mapping

  1. Changes
    - Simplify view to use p.* and add computed columns
    - This preserves PostgREST relationship mapping while still overriding values
    
  2. Approach
    - Use row_to_json to get all base columns
    - Override specific fields with active revision values
    - Reconstruct as a proper view
*/

-- Drop the problematic view
DROP VIEW IF EXISTS proposals_with_revision_count;

-- Create a simpler view that preserves relationships
...
-- Add a comment to help PostgREST identify the primary key
COMMENT ON VIEW proposals_with_revision_count IS 'Primary key: id. Foreign keys same as proposals table.';
