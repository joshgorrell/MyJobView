...
/*
  # Fix Proposal Approval Trigger - Remove Invalid Column Reference

  ## Problem
  The handle_unified_proposal_approval trigger function references NEW.assigned_to
  which does not exist on the proposals table. This causes the entire trigger to
  fail with a column-not-found error, which prevents sales orders, invoices, and
  projects from being created when a proposal is approved.

  ## Fix
  Replace NEW.assigned_to with just NEW.created_by in the notification insert,
  since assigned_to does not exist on proposals.
*/
