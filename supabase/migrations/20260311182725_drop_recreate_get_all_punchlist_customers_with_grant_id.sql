/*
  # Drop and recreate get_all_punchlist_customers with grant_id column

  ## Summary
  Drops and recreates the function to add grant_id to the return type.
  This is required because PostgreSQL does not allow changing an existing
  function's return type via CREATE OR REPLACE.

  ## Changes
  - Returns grant_id (uuid) — the punchlist_access_grants.id for grant-based
    access types (test_and_tune, promotional). NULL for vip_membership rows.
  - Returns the real stored status including 'suspended' instead of always
    computing 'active'/'expired' from the expiration date.
*/
