/*
  # Fix Revision Proposal Number Generation
  
  1. Issue
    - create_proposal_revision copies proposal_number from original
    - This violates unique constraint (company_id, proposal_number)
    
  2. Solution
    - Set proposal_number to NULL when creating revision
    - Let the generate_proposal_number trigger create a new unique number
    
  3. Security
    - No security changes, only fixes duplicate key constraint violation
*/
...
BEGIN
  ...
  -- NOTE: proposal_number is set to NULL so the trigger generates a new unique number
  ...
  SELECT
    ...
    contact_id,
    lead_id,
    NULL, -- Let trigger generate new proposal number
    title,
    status,
    valid_until,
    notes,
