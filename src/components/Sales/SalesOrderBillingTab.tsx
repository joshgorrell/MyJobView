      ...
      <div className="bg-gray-900/50 rounded-xl border border-gray-700/50 overflow-hidden">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-baseline gap-1.5 mt-0.5">
                ...
                {originalRemaining === 0 && originalTotal > 0 ? (
                  <span className="text-xs text-green-400/80">Fully invoiced</span>
                ) : (
                  <span className="text-xs text-gray-500">${fmt(originalTotal - originalRemaining)} invoiced</span>
                )}
              </div>
            </div>
            {originalRemaining > 0 ? (
              <button
                ...
                className="shrink-0 flex items-center gap-1 px-3 py-2 min-h-[36px] bg-blue-600 hover:bg-blue-500 active:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors touch-manipulation"
              >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-baseline gap-1.5 mt-0.5">
                    <span className={`text-base font-bold ${isNegCO ? 'text-red-400' : 'text-green-400'}`}>
                      {isNegCO ? `-$${fmt(Math.abs(co.change_amount || 0))}` : `+$${fmt(co.change_amount || 0)}`}
                    </span>
                    {(co.tax_amount || 0) > 0 && (
                      <span className="text-xs text-gray-500">+${fmt(co.tax_amount)} tax</span>
                    )}
                    {(co.tax_amount || 0) < 0 && (
                      <span className="text-xs text-gray-500">-${fmt(Math.abs(co.tax_amount))} tax</span>
                    )}
                  </div>
                  {absBilled > 0 && (
                ...
                {(hasUnbilledPositive || hasUnbilledCredit) && !isFullyBilled ? (
                  <button
                    ...
                    className="shrink-0 flex items-center gap-1 px-3 py-2 min-h-[36px] text-white text-xs font-semibold rounded-lg transition-colors touch-manipulation bg-blue-600 hover:bg-blue-500 active:bg-blue-700"
                  >
                    <DollarSign className="w-3 h-3" />
                    Bill
                  </button>
        ...
        <div className="px-4 py-4 bg-gray-800/30 border-t border-gray-600/50">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              <div className="text-sm font-semibold text-gray-300">Total Contract Value</div>
              <div className="text-xs text-gray-500 mt-0.5">
                ${fmt(originalTotal)} base
                {approvedCOs.length > 0 && (
                  <> + {approvedCOs.length} approved CO{approvedCOs.length > 1 ? 's' : ''}</>
                )}
              </div>
                ...
                <div className="mt-3">
                  <div className="flex items-center gap-2">
                    <div className="flex-1 h-2 bg-gray-700 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full transition-all ${remainingToInvoice === 0 ? 'bg-green-500' : 'bg-blue-500'}`}
                        style={{ width: `${Math.min(100, (totalInvoiced / fullContractWithCOs) * 100)}%` }}
                      />
                    </div>
                    <span className="text-xs text-gray-400 shrink-0">
                      {Math.min(100, Math.round((totalInvoiced / fullContractWithCOs) * 100))}%
                    </span>
                  </div>
                  {remainingToInvoice > 0 ? (
                    <div className="mt-1 text-xs text-amber-400/90 font-medium">
                      ${fmt(remainingToInvoice)} left to bill
                    </div>
                  ) : (
                    <div className="mt-1 text-xs text-green-400/80">Fully invoiced</div>
                  )}
                </div>
              )}
            </div>
            <div className="text-xl font-bold text-white shrink-0">${fmt(fullContractWithCOs)}</div>
          </div>
        </div>
      </div>
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <div className="flex items-baseline gap-1.5 mt-0.5">
                            ...
                            {entry.amount_type === 'percentage' && (
                              <span className="text-xs text-gray-500">{entry.amount}%</span>
                            )}
                          </div>
                        </div>
