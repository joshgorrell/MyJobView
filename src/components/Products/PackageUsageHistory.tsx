...
import { FileText, ShoppingCart, DollarSign, Package as PackageIcon, Calendar, User, TrendingUp, BarChart3 } from 'lucide-react';
import { supabase } from '../../lib/supabase';

interface PackageUsageHistoryProps {
  packageId: string;
}

interface UsageItem {
  id: string;
  type: 'proposal' | 'sales_order' | 'invoice';
  reference: string;
  customerName: string;
  quantity: number;
  unitPrice: number;
...
export function PackageUsageHistory({ packageId }: PackageUsageHistoryProps) {
  ...
  const loadUsageHistory = useCallback(async () => {
    ...
    try {
      ...
      const totalRev = items.reduce((sum, item) => sum + item.total, 0);
      ...
      console.error('Error loading package usage history:', error);
    } finally {
      setLoading(false);
    }
  }, [packageId]);
        ...
        <div className="divide-y divide-gray-200">
          {filteredHistory.length === 0 ? (
            <div className="p-8 text-center text-gray-500">
              ...
              <p className="text-sm mt-1">This package hasn't been used yet</p>
            </div>
          ) : (
