...
import { X, Package, Layers, Wrench, Zap } from 'lucide-react';

interface BulkUpdateProjectInfoModalProps {
...
export default function BulkUpdateProjectInfoModal({
  itemDescription,
  field,
  instanceCount,
  onUpdateSingle,
  ...
  onCancel,
  isLoading = false
}: BulkUpdateProjectInfoModalProps) {
  const fieldLabel = field === 'install' ? 'Install Notes' : 'Programming Info';
  const Icon = field === 'install' ? Wrench : Zap;
  const iconColor = field === 'install' ? 'text-cyan-400' : 'text-yellow-400';
        ...
        <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-4">
          ...
          <div className="bg-gray-900 rounded-lg p-4 border border-gray-700">
            ...
            <p className="text-xs sm:text-sm text-gray-400">
              {field === 'install'
                ? 'Installation notes and visibility settings will be updated.'
                : 'Programming labor hours, notes, and visibility settings will be updated.'}
            </p>
          </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-blue-900/20 border border-blue-700/50 rounded-lg p-3">
              <p className="text-xs text-blue-300">
                Updating all instances will apply the same {fieldLabel.toLowerCase()} to every occurrence of this product in the proposal.
              </p>
            </div>
          </div>
        </div>
