    ...
    <div className="relative w-full">
      ...
      {showCreate && <div className="fixed inset-0 z-[70] flex items-end justify-center bg-black/65 sm:items-center sm:p-4">
        <div className="flex max-h-[94dvh] w-full flex-col overflow-hidden rounded-t-2xl border border-gray-700 bg-gray-800 shadow-2xl sm:max-w-2xl sm:rounded-xl">
          <div className="flex items-center justify-between border-b border-gray-700 px-4 py-3 sm:px-6 sm:py-4"><div><h3 className="font-bold text-white">Add Location / Property</h3><p className="text-xs text-gray-400">Home, building, department, branch, office, campus, or any other customer location.</p></div><button type="button" onClick={() => setShowCreate(false)} className="rounded-lg p-2 text-gray-400 hover:bg-gray-700 hover:text-white"><X className="h-5 w-5" /></button></div>
          <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              ...
              <label><span className="mb-1 block text-xs font-semibold text-gray-300">Department</span><input value={form.department} onChange={e => setForm(f => ({ ...f, department: e.target.value }))} placeholder="Optional" className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white" /></label>
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-semibold text-gray-300">Building / Site Name</span><input value={form.building_name} onChange={e => setForm(f => ({ ...f, building_name: e.target.value }))} placeholder="Optional" className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white" /></label>
              ...
              <label><span className="mb-1 block text-xs font-semibold text-gray-300">Location Phone</span><input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))} className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white" /></label>
              <label><span className="mb-1 block text-xs font-semibold text-gray-300">Location Email</span><input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white" /></label>
              <label className="sm:col-span-2"><span className="mb-1 block text-xs font-semibold text-gray-300">Access Instructions</span><textarea value={form.access_instructions} onChange={e => setForm(f => ({ ...f, access_instructions: e.target.value }))} rows={2} placeholder="Gate code, parking, loading dock, keyholder, building access, etc." className="w-full rounded-lg border border-gray-700 bg-gray-900 px-3 py-2 text-sm text-white" /></label>
